...
# - First block has 17 messages, first 16 seem to have the same structure
# - Each write command seems to be part of one big message. If sent at a different time, the bytes might be shifted within one command. 
# - So it is necessary to read across the end of a command. Maybe some bytes at the end of each command has to be discarded
# - This message defines the foreground and background colors. For sure the entire screen background, except the frame along the four edges
# - Entire screen background (except frame): after 0x73, 0x79, 0xff
