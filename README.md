...
* length of each command always the same, 64 bytes including the leading 0xf2
* Setting text to the display starts with the first character in the top left corner, then continues along the first line, then second line, etc.
* if the text has reached the bottom right corner of the screen, fill up the last command with 0x00 until the length of 64 bytes is reached
* Example:
```
...
* three bytes for one character, starting from top-left display corner
...
* e2 86 90: left arrow
* e2 86 91: up arrow
* e2 86 92: right arrow
* e2 86 93: down arrow
* e2 96 b6: right closed triangle
* e2 97 80: left closed triangle
* e2 98 90: open square
* e2 ac a1: open rhombus
